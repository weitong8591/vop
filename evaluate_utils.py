...
try:
    ...
    from gluefactory.eval.utils import eval_poses_best
        ...
        with h5py.File(str(match_file), "r") as hfile, \
            ...
            for query in tqdm(self.image_list):
                ...
                for ref_name in refs:
                    ...
                    data["camera0"] = Camera.from_calibration_matrix(data["original_K0"])
                    data["T_w2cam0"] = Pose.from_4x4mat(data["T_w2cam0"])
                    data["camera1"] = Camera.from_calibration_matrix(data["original_K1"])
...
class Voting:
    """Overlap retrieval on db images, radius search + votings, return the overlap scores w/ and w/o TF-IDF
    weights."""
    ...
    def rerank(self, query_descriptors, db_descriptors, filtered_indices=None):
        ...
        # return voting results, in a shape of (#voting schemes, #queries, #db images)
        votings= np.zeros((2, query_descriptors.shape[0], db_descriptors.shape[0]))
        ...
        for i, query in tqdm(enumerate(query_normalized)):
            ...
            # w/o weights
            votings[0, i] = np.bincount(find_images, weights=similarity, minlength=votings.shape[-1])
            # with weights
            votings[1, i] = np.bincount(find_images, weights=weighted_similarity, minlength=votings.shape[-1])

        return {'scores': votings}
    ...
    def query_each(self, descriptors, filtered_indices=None):
        ...
        if filtered_indices is not None:
            ...
            for i, query in tqdm(enumerate(embeddings_normalized)):
                ...
                # tf-idf weights for each query patch
                ti = nid / n_d *np.log((N - 1) / (ni + self.EPS)) #if (N - 1) != ni else nid / n_d
                ti[ti<0] = 0
