...
outputs = Path(f"{opt.output_dir}/{opt.dataset}/{opt.method}{opt.num_loc}") if opt.method is not None else Path(f"{opt.output_dir}/{opt.dataset}/{opt.model}{opt.num_loc}")# where everything will be saved
...
print(f"Done, now you can submit {results} to LONG-TERM VISUAL LOCALIZATION benchmark!")
