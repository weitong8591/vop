...
def eval_poses(pose_results, auc_ths, key, unit="°"):
    ...
    for k, v in pose_results[best_th].items():
        ...
        summaries[f"mean_{k}"] = arr[arr!=np.inf].mean()
        summaries[f"max_{k}"] = arr[arr!=np.inf].max()
        summaries[f"min_{k}"] = arr[arr!=np.inf].min()
...
def eval_poses_best(pose_results, auc_ths, key, unit="°", n=445):
    ...
    mAAs = {k: np.mean(v) for k, v in pose_aucs.items()}
    best_th = max(mAAs, key=mAAs.get)
    best_indices = np.argmax(np.array(pose_results[best_th]['ransac_inl']).reshape(n, -1),axis=1)
    if len(pose_aucs) > -1:
        print("Tested ransac setup with following results:")
        print("AUC", pose_aucs)
        print("mAA", mAAs)
    ...
    aucs = AUCMetric(auc_ths, best_pose_error).compute()
    for i, ath in enumerate(auc_ths):
    ...
    for k, v in pose_results[best_th].items():
        ...
        if not np.issubdtype(np.array(v).dtype, np.number):
            continue
        summaries[f"m{k}"] = round(np.median(best_arr), 3)
        summaries[f"mean_{k}"] = best_arr[best_arr!=np.inf].mean()
        summaries[f"max_{k}"] = best_arr[best_arr!=np.inf].max()
        summaries[f"min_{k}"] = best_arr[best_arr!=np.inf].min()
