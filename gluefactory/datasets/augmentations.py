...
class FlipAugmentation(BaseAugmentation):
    ...
    def _init(self, conf):
        self.transforms = [
            A.HorizontalFlip(p=self.default_conf["p"])

        ]
        
class Rotate90Augmentation(BaseAugmentation):
    default_conf = {"p": 0.5}

    def _init(self, conf):
        self.transforms = [
            A.RandomRotate90(p=self.default_conf["p"])
...
class RotateAugmentation(BaseAugmentation):
    ...
    def _init(self, conf):
        self.transforms = [
            A.ShiftScaleRotate(p=self.default_conf["p"])
