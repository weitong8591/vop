...
class _PairDataset(torch.utils.data.Dataset):
    def __init__(self, conf, split, load_sample=True):
        ...
        ## load labels from depth and roma
        self.keypoint_centers = np.load('dumped_data/14patch_centers.npz')['keypoints'].__array__()
        ...
        self.gt_labels = h5py.File(f"{self.conf.gt_label_path}/supervision_{self.split}_check.h5", 'r')
    ...
    def sample_new_items(self, seed):
        ...
        if isinstance(num_per_scene, Iterable):
            ...
            num_neg = None
        if self.conf[split + "_pairs"] is not None:#split != "train" and
            ...
            for line in pairs_path.read_text().rstrip("\n").split("\n"):
                im0, im1, overlap_saved= line.split(" ")
                scene = im0.split("/")[0]
                ...
                self.items.append((scene, idx0, idx1, float(overlap_saved)))
            ...
            for scene in self.scenes:
                ...
                info = np.load(str(path), allow_pickle=True) #['image_paths', 'depth_paths', 'intrinsics', 'poses', 'overlap_matrix', 'scale_ratio_matrix', 'angles', 'n_points3D', 'points3D_id_to_2D', 'points3D_id_to_ndepth']
                num = self.conf[self.split + "_num_per_scene"]

                if "pair_infos" in info.keys():
                    # LoFTR format
                    pairs = np.array(info["pair_infos"], dtype="object")
                    # import pdb; pdb.set_trace()
                    if num is not None and len(pairs) > num:
                        selected = np.random.RandomState(seed).choice(
                            len(pairs), num, replace=False
                        )
                        pairs = pairs[selected]
                    for (idx0, idx1), overlap, _ in pairs:
                        self.items.append((scene, idx0, idx1, overlap))
                else:
                    valid = (self.images[scene] != None) & (  # noqa: E711
                        self.depths[scene] != None  # noqa: E711
                    )
                    ...
                    if num_neg is not None:
                        neg_pairs = np.stack(np.where(mat == 0.0), -1)# remove the image itself from negative pairs
    ...
    def _read_view(self, scene, idx):
        ...
        if self.conf.read_depth:
            depth_path = self.root / self.depths[scene][idx]
            # depth_path = (
    ...
    def getitem(self, idx):
        if self.conf.views == 2:
            ...
            data["gt_labels"] = torch.zeros(1, 16**2, 16**2)
            data["label_confs"] = torch.zeros_like(data["gt_labels"], dtype=torch.int16)

            pair_key = f"{data0['scene']}_images_{data0['name']}_{data1['scene']}_images_{data1['name']}"
            gt_indices = self.gt_labels[pair_key]['gt_labels'].__array__()
            roma_indices = self.gt_labels[pair_key]['gt_labels_roma'].__array__()
            gt_coords = np.vstack(gt_indices[0].nonzero()).T
            roma_coords = np.vstack(roma_indices[0].nonzero()).T

            if overlap > 0:
                if self.conf.combine:
                    gt_coords_view = np.ascontiguousarray(gt_coords).view([('', gt_coords.dtype)] * gt_coords.shape[1])
                    roma_coords_view = np.ascontiguousarray(roma_coords).view([('', roma_coords.dtype)] * roma_coords.shape[1])

                    common_coords = np.intersect1d(roma_coords_view, gt_coords_view)
                    common_labels = common_coords.view(roma_coords.dtype).reshape(-1, 2)

                    data["gt_labels"][0][common_labels[:, -2], common_labels[:, -1]] = 1
                    data["label_confs"][0, common_labels[:, -2], common_labels[:, -1]] = torch.from_numpy(self.gt_labels[pair_key]['label_confs'].__array__()[0, common_labels[:, -2], common_labels[:, -1]])

                elif self.conf.roma_supervision:
                    data["gt_labels"][0][roma_coords[:, -2], roma_coords[:, -1]] = 1
                    data["label_confs"][0, roma_coords[:, -2], roma_coords[:, -1]] = torch.from_numpy(self.gt_labels[pair_key]['label_confs_roma'].__array__()[0, roma_coords[:, -2], roma_coords[:, -1]])

                else:
                    data["gt_labels"][0][gt_coords[:, -2], gt_coords[:, -1]] = 1
                    data["label_confs"] = torch.from_numpy(self.gt_labels[pair_key]['label_confs'].__array__())

                # flip the labels
                if self.photo_augment.conf['name'] == 'flip':
            ...
            # no overlap
                ...
                data["neg_labels"] = torch.ones_like(data["gt_labels"])
                diagonal = torch.eye(256, 256).bool()
                data["neg_labels"].masked_fill_(diagonal, 0.)
            ...
            for scene in self.scenes:
                ...
                if "pairs" in info.keys():
                    raise NotImplementedError(
                        "Triplets currently not supported on LoFTR scene files"
                    )
                else:
                    valid = (self.images[scene] != None) & (  # noqa: E711
