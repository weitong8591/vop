...
def precalculate_patch_colors(descriptors):
    """
    Use PCA to reduce the dimensionality of the patch descriptors to 3
    and normalize these for RGB color space.
...
def process_image_into_patches(image, patch_height, patch_width, gap_size):
    """
    Divides an image into patches with specified height and width, including gaps around them, 
...
def create_composite_image(image1, image2, gap_between_images, orientation='vertical'):
    """
    Combines two images with a specified gap between them into a single composite image,
    either horizontally or vertically based on the orientation parameter.
...
def draw_connections_with_numbers(composite_image, matches, patch_height, patch_width, gap_size, gap_between_images, fontsize=18, orientation='horizontal'):
    """
    Places a number in the middle of corresponding patches across two processed images
    within a composite image, based on a list of matches in row-major order. Supports both
